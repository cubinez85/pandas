...
#
#
# orders_path = './data_2/Shop_orders.csv'
# output_dir = './'
#
# if not os.path.exists(output_dir):
#     os.makedirs(output_dir)
...
#     try:
#         df_order = pd.read_csv(file_path)
#         return df_order
#     except FileNotFoundError:
#         print(f'Error: File "{file_path}" not found')
#         return None
...
#
#
# # df = read_csv_data(orders_path)
# # print(df)
#
# def rename_indexes(df_order):
#     df_order.index = ['a', 'b', 'c', 'd', 'e', 'f', 'g'][:len(df_order)]
#     return df_order
#
#
# # df = rename_indexes(df_order=read_csv_data(orders_path))
# # print(df)
#
# def save(df, csv_path, excel_path, json_path):
#     try:
#         if df is not None:
#             df.to_csv(csv_path, index=False)
#             df.to_excel(excel_path, index=True)
#             df.to_json(json_path, index=True)
#             return 'Файлы успешно сохранены'
#         else:
#             return 'Не удалось сохранить файлы: Data Frame is None'
#     except Exception as e:
#         return f'Error: "{e}"'
#
#
# def etl():
#     df_order = read_csv_data(orders_path)
#     if df_order is not None:
#         new_indexes = rename_indexes(df_order)
#         if new_indexes is not None:
#             csv_path = os.path.join(output_dir, 'Orders_keys.csv')
#             excel_path = os.path.join(output_dir, 'Orders_keys.xlsx')
#             json_path = os.path.join(output_dir, 'Orders_keys.json')
#             result = save(new_indexes, csv_path, excel_path, json_path)
#             return result
#         else:
#             return 'Error during rename_indexes function'
#     else:
#         return 'Error during read_csv_data function'
#
#
# print(etl())
